  ...
  getAttributeDescriptions() {
    std::array<VkVertexInputAttributeDescription, 2> attributeDescriptions{
        {{.location = 0,
          ...
          .offset = offsetof(VertexWithColor, pos)},
         {.location = 1,
          .binding = 0,
          .format = VK_FORMAT_R32G32B32_SFLOAT,
          .offset = offsetof(VertexWithColor, color)}}};
...
struct VulkanApp::VulkanInnards {
  ...
  std::vector<VertexWithColor> vertices{{{0.0f, -0.5f}, {1.0f, 0.0f, 0.0f}},
                                        {{0.5f, 0.5f}, {0.0f, 1.0f, 0.0f}},
                                        {{-0.5f, 0.5f}, {0.f, 0.f, 1.f}}};
};

VulkanApp::VulkanApp()
    : vi(std::make_unique<VulkanInnards>()) {}
...
void VulkanApp::cleanup() {
  ...
  vkDestroyPipeline(vi->device, vi->graphicsPipeline, nullptr);
  vkDestroyPipelineLayout(vi->device, vi->pipelineLayout, nullptr);
  vkDestroyRenderPass(vi->device, vi->renderPass, nullptr);
  cleanupSwapChain();
  vkDestroyBuffer(vi->device, vi->stagingBuffer, nullptr);
  vkFreeMemory(vi->device, vi->stagingBufferMemory, nullptr);
  vkDestroyBuffer(vi->device, vi->vertexBuffer, nullptr);
  vkFreeMemory(vi->device, vi->vertexBufferMemory, nullptr);
...
void VulkanApp::createLogicalDevice() {
  ...
  vkGetDeviceQueue(vi->device, vi->presentationQueueNum, 0, &vi->presentationQueue);
}
...
void VulkanApp::createSwapChain() {
  ...
  vi->swapChainExtent = chooseSwapExtent(windowWidth, windowHeight, vi->swapChainDetails.capabilities);
  ...
  uint32_t queueFamilyIndices[] = {vi->graphicsQueueNum, vi->presentationQueueNum};
...
void VulkanApp::createImageViews() {
  ...
  for (size_t i = 0; i < vi->swapChainImages.size(); ++i) {
    VkImageViewCreateInfo createInfo{
        ...
        .image = vi->swapChainImages[i],
        .viewType = VK_IMAGE_VIEW_TYPE_2D,
        .format = vi->swapChainImageFormat};
    ...
    if (vkCreateImageView(vi->device, &createInfo, nullptr,
                          &vi->swapChainImageViews[i]) != VK_SUCCESS) {
...
void VulkanApp::createRenderPass() {
  ...
  if (vkCreateRenderPass(vi->device, &renderPassInfo, nullptr, &vi->renderPass) !=
      VK_SUCCESS) {
...
void VulkanApp::createGraphicsPipeline() {
  VkShaderModule vertShader = loadShaderFromFile(vi->device, "vulkan_demo_vert");
  VkShaderModule fragShader = loadShaderFromFile(vi->device, "vulkan_demo_frag");
  ...
  auto bindingDescription = VertexWithColor::getBindingDescription();
  auto attributeDescriptions = VertexWithColor::getAttributeDescriptions();
  ...
  VkViewport viewport{.x = 0.0f,
                      .y = 0.0f,
                      .width = (float)vi->swapChainExtent.width,
                      .height = (float)vi->swapChainExtent.height,
                      .minDepth = 0.0f,
                      .maxDepth = 1.0f};

  VkRect2D scissor{.offset = {0, 0}, .extent = vi->swapChainExtent};
  ...
  if (vkCreatePipelineLayout(vi->device, &pipelineLayoutInfo, nullptr,
                             &vi->pipelineLayout) != VK_SUCCESS) {
  ...
  VkGraphicsPipelineCreateInfo pipelineInfo{
      ...
      .layout = vi->pipelineLayout,
      .renderPass = vi->renderPass,
      .subpass = 0,
      .basePipelineHandle = VK_NULL_HANDLE,
      .basePipelineIndex = -1};
  if (vkCreateGraphicsPipelines(vi->device, VK_NULL_HANDLE, 1, &pipelineInfo,
                                nullptr, &vi->graphicsPipeline) != VK_SUCCESS) {
    throw std::runtime_error("failed to create graphics pipeline!");
  }

  vkDestroyShaderModule(vi->device, vertShader, nullptr);
  vkDestroyShaderModule(vi->device, fragShader, nullptr);
}
...
void VulkanApp::createVertexBuffer() {
  ...
  createBuffer(vi->physicalDevice, vi->device, bufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
               VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
                   VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
               vi->stagingBuffer, vi->stagingBufferMemory);

  // void *data;
  // vkMapMemory(device, stagingBufferMemory, 0, bufferSize, 0, &data);
  // memcpy(data, vi->vertices.data(), (size_t)bufferSize);
  // vkUnmapMemory(device, stagingBufferMemory);
  createBuffer(vi->physicalDevice, vi->device,
      bufferSize,
      VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
      VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, vi->vertexBuffer, vi->vertexBufferMemory);
...
void VulkanApp::createCommandBuffers() {
  ...
  if (vkAllocateCommandBuffers(vi->device, &allocInfo, vi->commandBuffers.data()) !=
      VK_SUCCESS) {
...
void VulkanApp::bufferVertexData() {
  VkDeviceSize bufferSize = sizeof(vi->vertices[0]) * vi->vertices.size();
  void *data;
  vkMapMemory(vi->device, vi->stagingBufferMemory, 0, bufferSize, 0, &data);
  memcpy(data, vi->vertices.data(), (size_t)bufferSize);
  vkUnmapMemory(vi->device, vi->stagingBufferMemory);
  copyBuffer(vi->device, vi->commandPool, vi->graphicsQueue, vi->stagingBuffer, vi->vertexBuffer, bufferSize);
}
...
void VulkanApp::createSyncObjects() {
  ...
  for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i) {
    if (vkCreateSemaphore(vi->device, &semaphoreInfo, nullptr,
        ...
        vkCreateFence(vi->device, &fenceInfo, nullptr, &vi->inFlightFences[i]) !=
            VK_SUCCESS) {
...
void VulkanApp::drawFrame() {
  ...
  vi->vertices[0].color = {1.0f, color, color};
  vi->vertices[1].color = {color, 1.0f, color};
  vi->vertices[2].color = {color, color, 1.0f};
  bufferVertexData();
  uint32_t imageIndex;
  VkResult result = vkAcquireNextImageKHR(
      vi->device, vi->swapChain, UINT64_MAX, vi->imageAvailableSemaphores[currentFrame],
  ...
  recordCommandBuffer(
  vi->renderPass,
  vi->swapChainFramebuffers,
  vi->swapChainExtent,
  vi->graphicsPipeline,
  vi->vertexBuffer,
  vi->commandBuffers[currentFrame], imageIndex);
