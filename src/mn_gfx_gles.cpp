...
Tex::Tex(const char* path) {
  ...
  size_t width = 32;
  size_t height = 64;
  size_t buffer_size = width * height * 2 + 0x8A; // 0x8a because of bmp header
  std::vector<unsigned char> data(buffer_size);
  fread(data.data(), 1, width * height *2 + 0x8A, tex_file);
  fclose(tex_file);
  glGenTextures(1, &id);
  for(int i =0; i < width*height; i++) {
    //bmp bits are misplaced unfortunately
    unsigned short *sh = &((unsigned short*)(data.data() + 138))[i];
    *sh = (*sh << 1) | (((0xffff^(*sh)) & 0x8000) >> 15);
  }
  ...
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_BASE_LEVEL, 0);
    glTexImage2D(
        ...
        (unsigned short*)(data.data()+ 0x8a));
  secret_texture_info = (uint64_t)id;
};
...
std::shared_ptr<texture> texture::create_from_file(
  ...
  size_t width = 32;
  size_t height = 64;
  size_t buffer_size = width * height * 2 + 0x8A; // 0x8a because of bmpq
  std::vector<unsigned char> data(buffer_size);
  fread(data.data(), 1, width * height *2 + 0x8A, tex_file);
  fclose(tex_file);
  glGenTextures(1, &id);
  for(int i =0; i < width*height; i++) {
    //bmp bits are misplaced
    unsigned short *sh = &((unsigned short*)(data.data() + 138))[i];
    *sh = (*sh << 1) | (((0xffff^(*sh)) & 0x8000) >> 15);
  }
  glBindTexture(GL_TEXTURE_2D, id);
...
void draw_platform(
  ...
  glDisable(GL_BLEND);
  glUseProgram(polygon_fill_prog->id());
  glEnableVertexAttribArray(polygon_fill_prog->v_pos());
  glBindBuffer(GL_ARRAY_BUFFER, corner_vertex_buffer->id());
  glVertexAttribPointer(polygon_fill_prog->v_pos(), 2, GL_FLOAT, GL_FALSE, 0, nullptr);
  glUniform1i(polygon_fill_prog->u_texture(), 0);
  glEnable(GL_BLEND);
...
void draw_sprite(
  ...
  fvec2 view,
  const std::shared_ptr<buffer>& the_stupid_fucking_square
) {
