...
void gazo::init() {
  ...
  mapping = (glm::vec2*) malloc(n_verts * sizeof(glm::vec2));
  pos20 = (float*) malloc(n_verts * 2 * sizeof(float));


  pos                           = (glm::vec2*) malloc(n_verts * sizeof(glm::vec2));
  vel                           = (glm::vec2*) malloc(n_verts * sizeof(glm::vec2));
  sample_pos                    = (glm::vec2*) malloc(n_verts * sizeof(glm::vec2));
  sample_vel                    = (glm::vec2*) malloc(n_verts * sizeof(glm::vec2));
  acc                           = (glm::vec2*) malloc(n_verts * sizeof(glm::vec2));
  memset(acc, 0, n_verts * sizeof(glm::vec2)); // 0x0000000000000000 is equal to 0.0
  delta_pos                     = (glm::vec2*) malloc(n_verts * sizeof(glm::vec2));
  delta_vel                     = (glm::vec2*) malloc(n_verts * sizeof(glm::vec2));
  ...
  for(int i = 0; i < 9; i++) {
    ...
    for(int j = 0; j < n_sides; j++) {
      glm::vec2 mapping_here = mapping[j + 1];
      glm::vec2 uv_value = {
  ...
  //glBufferData(GL_ELEMENT_ARRAY_BUFFER, n_sides * 3 * sizeof(uint16_t), elements, GL_STATIC_DRAW);

  for(uint32_t i = 0u; i < n_verts * 2u; i++) {
    ((double*)pos)[i] = ((double*)mapping)[i] * radius;
  }
  for(uint32_t i = 0u; i < n_verts; i++) {
    pos[i].y++;
    vel[i].x = 0.0;
    vel[i].y = 0.0;
  }
...
void gazo::push_out_from_platform(double interval, platform* pltfm) {
  for(int i = 0; i < n_verts; i++) {
    ...
    if(pltfm->can_we_like_can_we_please_like_put_stuff_here_at_this_location_x_and_y_please_or_is_that_like_a_not_good_place_to_put_stuff_because_like_you_cant_put_stuff_there(p)) {
      glm::vec2 displacement = pltfm->shortest_path(p);
      pos[i].x += displacement.x;
      pos[i].y += displacement.y;
      glm::vec2 velocity_change = glm::vec2{displacement.x / interval, displacement.y / interval};
      double displacement_length = hypot(displacement.x, displacement.y);
      glm::vec2 normal = glm::vec2{displacement.x / displacement_length, displacement.y / displacement_length};
...
void gazo::calculate_acc(glm::vec2* pos_in, glm::vec2* vel_in, glm::vec2* acc_out) {
  for(int i = 0; i < n_verts; i++) {
  ...
  glm::vec2 glaggle_rotation = {0, 0}; //as like a complex number.
  //for example {0, 1} means rotated counterclokwise 90 degress
  glm::vec2 glaggle_center = pos_in[0];
  ...
  for(int i = 0; i < n_sides; i++) {
    ...
    glm::vec2 endpoint_velocities[2] = {
